...
`;
	...
	$effect(() => {
		if ((mouseX || mouseY || isActive || isPinned) && gl) {
			render();
		}
	});
	...
	function togglePin(e: MouseEvent | TouchEvent) {
		const clientX = "clientX" in e ? (e as MouseEvent).clientX : (e as TouchEvent).touches[0]?.clientX || (e as TouchEvent).changedTouches[0]?.clientX;
		const clientY = "clientY" in e ? (e as MouseEvent).clientY : (e as TouchEvent).touches[0]?.clientY || (e as TouchEvent).changedTouches[0]?.clientY;
