...
MOCK_STATUS_FILE="$TEST_DIR/mock_status"
...
# Test 2: Failure with uncommitted modified changes
echo "Running Test 2: Uncommitted modified changes detected (expecting failure)..."
echo "M src/index.ts" > "$MOCK_STATUS_FILE"
...
# Test 3: Failure with untracked files
echo "Running Test 3: Untracked files detected (expecting failure)..."
echo "?? src/new_file.ts" > "$MOCK_STATUS_FILE"
if bash scripts/conductor-verify.sh 2> "$TEST_DIR/stderr"; then
