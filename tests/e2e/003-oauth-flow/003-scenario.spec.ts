...
test("GitHub OAuth Flow", async ({ page }, testInfo) => {
	...
	await helper.step("landing_page_loaded", {
		...
		verifications: [
			...
			},
			{
				...
				check: async () => expect(page).toHaveTitle("Conductor Observability"),
			},
		],
	});
	...
	await helper.step("logged_in_profile_visible", {
		...
		verifications: [
			{
				spec: "Username is visible",
				check: async () => expect(page.getByText("testuser")).toBeVisible(),
			},
			{
				spec: "Avatar is visible",
				check: async () => expect(page.getByAltText("testuser")).toBeVisible(),
			},
			{
				spec: "GitHub API Verified message is visible",
				check: async () =>
					expect(page.getByText("GitHub API Verified ✅")).toBeVisible(),
			},
			{
				spec: "Repo name is visible",
				check: async () =>
					expect(page.getByText("LLM-Orchestration/conductor")).toBeVisible(),
			},
			{
				spec: "Logout button is visible",
				check: async () =>
	...
	await helper.step("logged_out", {
		...
		verifications: [
			...
			{
				spec: "Username is not visible",
				check: async () => expect(page.getByText("testuser")).not.toBeVisible(),
			},
		],
	});
