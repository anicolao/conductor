...
test('PR Lookup by Workflow Run Data', async ({ page }, testInfo) => {
  const helper = new TestStepHelper(page, testInfo);
  helper.setMetadata('PR Lookup by Workflow Run Data', 'Verify that the PR is correctly identified using the pull_requests field already present in the workflow run object.');
  ...
  const mockWorkflowRuns = [
    {
      ...
      status: 'completed',
      conclusion: 'success',
      html_url: 'https://github.com/LLM-Orchestration/conductor/actions/runs/777888999',
  ...
  // Mock GitHub Issues API
  await page.route('https://api.github.com/repos/LLM-Orchestration/conductor/issues/200', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await helper.step('pr_link_visible_by_direct_data', {
    description: 'PR link is visible based on the direct pull_requests data in the workflow run',
    verifications: [
      {
        spec: 'PR link points to the correct PR found via direct data',
        check: async () => {
          const row = page.getByRole('row').filter({ hasText: 'LLM-Orchestration/conductor' });
          await expect(row.getByRole('link', { name: 'PR #201' })).toHaveAttribute('href', 'https://github.com/LLM-Orchestration/conductor/pull/201');
        }
      }
    ]
  });
