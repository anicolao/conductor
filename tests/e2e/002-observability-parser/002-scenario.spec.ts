...
test('Observability Log Parser Debug Page', async ({ page }, testInfo) => {
  ...
  helper.setMetadata('Observability Log Parser Debug Page', 'Verify that the log parser correctly extracts and displays events.');
  ...
  await helper.step('debug_page_loaded', {
    ...
    verifications: [
      ...
      },
      {
        ...
        check: async () => {
          await expect(page.getByPlaceholder('Paste your logs here')).toBeVisible();
        }
      }
    ]
  });
