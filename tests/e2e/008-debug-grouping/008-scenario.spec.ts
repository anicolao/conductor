		...
		verifications: [
			...
			},
			{
				...
				check: async () => {
					...
					await expect(
						firstGroup.getByText("DEBUG MESSAGES (3)"),
					).toBeVisible();
					const isOpen = await firstGroup.evaluate(
			...
			},
			{
				...
				check: async () => {
					...
					await expect(
						secondGroup.getByText("DEBUG MESSAGES (2)"),
					).toBeVisible();
			...
			},
			{
				...
				check: async () => {
					...
					await expect(
						thirdGroup.getByText("DEBUG MESSAGES (3)"),
					).toBeVisible();
				},
			},
		],
		...
		verifications: [
			{
				...
				check: async () => {
					const thirdGroup = page.locator(".log_debug_group").nth(2);
					const isOpen = await thirdGroup.evaluate(
						(el: HTMLDetailsElement) => el.open,
					);
					expect(isOpen).toBe(true);
					await expect(
						page.getByText("I am a message bus message"),
					).toBeVisible();
