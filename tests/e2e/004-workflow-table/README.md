# Workflow Table Display

Verify that the landing page shows a table of recent workflow runs when logged in.

## Landing page loaded with Login button

![Landing page loaded with Login button](./screenshots/000-landing-page-loaded.png)

### Verifications
- [x] Login button is visible

---

## Workflow table is displayed with correct data

![Workflow table is displayed with correct data](./screenshots/001-workflow-table-visible.png)

### Verifications
- [x] Table headers are correct
- [x] First row data is correct
- [x] Second row data is correct

---

