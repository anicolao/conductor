...
test("Workflow Table Display", async ({ page }, testInfo) => {
	...
	const mockWorkflowRuns = [
		{
			...
			conclusion: "success",
			html_url:
				"https://github.com/LLM-Orchestration/conductor/actions/runs/123456789",
		...
		},
		{
			...
			conclusion: null,
			html_url:
				"https://github.com/some-org/another-repo/actions/runs/987654321",
	...
	await page.route(
		"https://api.github.com/repos/LLM-Orchestration/conductor/issues/83",
		async (route) => {
			await route.fulfill({
				status: 200,
				contentType: "application/json",
	...
	await page.route(
		"https://api.github.com/repos/some-org/another-repo/issues/42",
		async (route) => {
			await route.fulfill({
				status: 200,
				contentType: "application/json",
		...
		verifications: [
			{
				...
				check: async () => {
					await expect(
						page.getByRole("columnheader", { name: "Repository" }),
					).toBeVisible();
					await expect(
						page.getByRole("columnheader", { name: "Issue" }),
					).toBeVisible();
					await expect(
						page.getByRole("columnheader", { name: "PR" }),
					).toBeVisible();
					await expect(
						page.getByRole("columnheader", { name: "Workflow Run" }),
					).toBeVisible();
					await expect(
			...
			},
			{
				...
				check: async () => {
					...
					await expect(
						row.getByRole("link", { name: "LLM-Orchestration/conductor" }),
					).toHaveAttribute(
						"href",
						"https://github.com/LLM-Orchestration/conductor",
					);
					// Wait for title to be loaded
					await expect(
						row.getByRole("link", { name: "#83: Fix the landing page table" }),
					).toBeVisible();
					await expect(
						row.getByRole("link", { name: "PR #83" }),
					).toHaveAttribute(
						"href",
						"https://github.com/LLM-Orchestration/conductor/pull/84",
					);
					await expect(
						row.getByRole("link", { name: "complete" }),
					).toHaveAttribute("href", /.*\/run\?id=123456789$/);
				},
			},
			{
				...
				check: async () => {
					const row = page
						.getByRole("row")
						.filter({ hasText: "some-org/another-repo" });
					await expect(
						row.getByRole("link", { name: "some-org/another-repo" }),
					).toHaveAttribute("href", "https://github.com/some-org/another-repo");
					// Wait for title to be loaded
					await expect(
						row.getByRole("link", { name: "#42: Meaning of life" }),
					).toBeVisible();
					await expect(
						row.getByRole("link", { name: /PR #/ }),
					).not.toBeVisible();
					await expect(
						row.getByRole("link", { name: "in progress" }),
					).toHaveAttribute("href", /.*\/run\?id=987654321$/);
				},
			},
		],
	});
