	...
	await helper.step("gemini_events_rendered", {
		...
		verifications: [
			...
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("I will help you with that."),
					).toBeVisible();
				},
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("Tool Use: read_file", { exact: true }),
					).toBeVisible();
			...
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("Tool Use: list_directory", { exact: true }),
					).toBeVisible();
					await expect(page.getByText("(1o6dfh4u)").first()).toBeVisible();
					await expect(
						page.getByText(
							'"dir_path": "observability-ui/src/lib/components/"',
						),
					).toBeVisible();
				},
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("Unknown Event: unknown_event"),
					).toBeVisible();
			...
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("TOOL RESULT: read_file (success)", { exact: true }),
					).toBeVisible();
			...
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("TOOL RESULT: run_shell_command (success)", {
							exact: true,
						}),
					).toBeVisible();
					await expect(
						page.getByText("Status: success", { exact: true }).first(),
					).toBeVisible();
			...
			},
			{
				...
				check: async () => {
					await expect(
						page.getByText("TOOL RESULT: list_directory (COMPLETED)", {
							exact: true,
						}),
					).toBeVisible();
