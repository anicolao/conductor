	...
	await helper.step("run_details_loaded", {
		...
		verifications: [
				...
				check: async () =>
					expect(
						page.getByRole("heading", {
							name: `Run Details: Conductor [LLM-Orchestration/conductor] Issue #88`,
						}),
			...
			},
			{
				...
				check: async () =>
					expect(page.locator(".status-badge.success")).toContainText(
						"success",
					),
			},
			{
				...
				check: async () =>
					expect(
						page.getByRole("link", { name: "View on GitHub (Drill Down) ↗" }),
					).toHaveAttribute(
						"href",
						`https://github.com/LLM-Orchestration/conductor/actions/runs/${runId}`,
					),
			},
		],
	});
		...
		verifications: [
			{
				...
				check: async () => {
					...
					await expect(terminal.locator(".terminal-line.stdout")).toContainText(
						"Compiling source code...",
					);
					await expect(terminal.locator(".terminal-line.stderr")).toContainText(
			...
			},
			{
				...
				check: async () => {
					...
					await expect(sessionStart.locator(".event-type")).toHaveText(
						"Session Started",
					);
					await expect(sessionStart.locator(".event-body")).toContainText(
						"Branch: main",
					);

					const logInfo = page.locator(".event-card.log_info");
					await expect(logInfo.locator(".event-type")).toHaveText("INFO");
					await expect(logInfo.locator(".event-body")).toContainText(
						"Build started",
					);

					const sessionEnd = page.locator(".event-card.session-end.success");
					await expect(sessionEnd.locator(".event-type")).toContainText(
			...
			},
			{
				...
				check: async () => {
					...
					await expect(event.locator(".task-content")).toContainText(
						"Implement feature",
					);
				},
			},
