...
test('Markdown Support in Gemini Events', async ({ page }, testInfo) => {
  ...
  helper.setMetadata('Markdown Support', 'Verify that Gemini messages and results are rendered as markdown.');
...
2026-04-18T10:00:05Z ::CONDUCTOR_EVENT:: ${JSON.stringify(event2)}
  ...
  await expect(page.locator('.gemini-event')).toHaveCount(2, { timeout: 15000 });
    ...
    verifications: [
      {
        ...
        check: async () => {
          ...
          await expect(userMessage.locator('pre code')).toContainText("console.log('hello');");
        }
      },
      {
