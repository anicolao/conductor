...
describe('JSON mode and Debug intercept', () => {
  ...
  it('should intercept GEMINI_EVENT from stdout', async () => {
    ...
    const json = JSON.stringify({ type: 'init', sessionId: '123', model: 'gemini-pro' });
    await runStreamingCommand('sh', ['-c', `echo '${json}'`], process.env);

    expect(logEvent).toHaveBeenCalledWith('GEMINI_EVENT', expect.objectContaining({
      type: 'init',
      sessionId: '123'
    }));
  ...
  it('should fallback to regular stdout if not valid JSON or missing type', async () => {
    ...
    expect(logEvent).not.toHaveBeenCalledWith('GEMINI_EVENT', expect.any(Object));

    vi.clearAllMocks();
    await runStreamingCommand('sh', ['-c', 'echo "{\\"no\\": \\"type\\"}"'], process.env);
    expect(logger.stdout).toHaveBeenCalled();
    expect(logEvent).not.toHaveBeenCalledWith('GEMINI_EVENT', expect.any(Object));
  });

  it('should intercept debug logs from stderr', async () => {
    await runStreamingCommand('sh', ['-c', 'echo "[Routing] using model X" >&2'], process.env);
    expect(logEvent).toHaveBeenCalledWith('LOG_DEBUG', { message: '[Routing] using model X' });
    expect(logger.stderr).not.toHaveBeenCalled();

    vi.clearAllMocks();
    await runStreamingCommand('sh', ['-c', 'echo "[Memory] current size" >&2'], process.env);
    expect(logEvent).toHaveBeenCalledWith('LOG_DEBUG', { message: '[Memory] current size' });

    vi.clearAllMocks();
    await runStreamingCommand('sh', ['-c', 'echo "[Status] thinking" >&2'], process.env);
    expect(logEvent).toHaveBeenCalledWith('LOG_DEBUG', { message: '[Status] thinking' });
  });

  it('should fallback to regular stderr for other logs', async () => {
    await runStreamingCommand('sh', ['-c', 'echo "some error" >&2'], process.env);
