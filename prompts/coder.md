...
3. **Commit**: Create commits using `npm run commit -- <issue_number> "<message>"`. This ensures the issue is linked to every commit. Push changes to the current feature branch.
4. **Report**: When done:
   - Ensure your summary includes a reference to the issue (e.g., "Closes #<issue_number>") to assist the `@conductor` in PR creation.
   - You MUST commit all your changes using `npm run commit` before handoff.
   - Hand off by running:
