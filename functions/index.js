...
async function dispatchProjectActivation(
	repository,
	issueNumber,
	token,
	eventName,
...
exports.githubProjectsV2Webhook = onRequest(
	...
	async (req, res) => {
		...
		if (
			!verifyWebhookSignature(rawBody, signature, githubWebhookSecret.value())
		) {
		...
		try {
          ...
          node(id: $id) {
			...
			if (!isStatusTrigger && !isPersonaTrigger) {
				logger.info(
					"Ignoring project item event: neither status trigger nor persona trigger met",
					{
						deliveryId,
						issueNumber,
						action,
			...
			logger.info("Dispatched project activation", {
				deliveryId,
				repositoryName,
				issueNumber,
				statusName,
