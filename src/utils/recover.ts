...
export const ConductorWorkflowRunSchema = z
	.object({
		status: z.string(),
...
export function toProjectIssueItem(
	...
	return {
		...
		status,
		persona:
			node.persona?.name === "coder" || node.persona?.name === "conductor"
				? node.persona.name
				: null,
	};
}
...
export function hasActiveRun(
	...
	return runs.some((run) => {
		...
		return (
			...
			target.issueNumber === item.issueNumber
		);
	});
}

export function countRecoveryAttempts(
	item: ProjectIssueItem,
	runs: ConductorWorkflowRun[],
): number {
...
export function findOrphanedItems(
	...
	return items.filter(
		(item) => item.status === "In Progress" && !hasActiveRun(item, runs),
	);
}
